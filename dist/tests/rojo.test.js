...
test("RojoSnapshotBuilder builds instances from default.project.json and files", async () => {
    ...
    fs.writeFileSync(path.join(tmp, "default.project.json"), JSON.stringify(project, null, 2), "utf8");
    const builder = new RojoSnapshotBuilder({
        cwd: tmp,
        projectFile: "default.project.json",
    });
    const instances = await builder.build();
    const script = instances.find((i) => i.path.join("/") === "ReplicatedStorage/Modules/Foo");
...
test("RojoSnapshotBuilder handles init.luau in a folder as an init script", async () => {
    ...
    const instances = await builder.build();
    const inst = instances.find((i) => i.path.join("/") === "ReplicatedStorage/Modules/MyModule");
...
`;
    ...
    assert.ok(part?.properties &&
        part.properties.Size &&
        part.properties.Size.__type === "Vector3");
    ...
    assert.ok(label?.properties &&
        label.properties.FontFace &&
        label.properties.FontFace.__type === "Font");
    const particles = instances.find((i) => i.name === "TestParticles");
    assert.ok(particles, "TestParticles emitted");
    assert.ok(particles?.properties &&
        particles.properties.Color &&
        particles.properties.Color.__type === "ColorSequence");
    const implicit = instances.find((i) => i.name === "ImplicitPart");
    assert.ok(implicit, "ImplicitPart emitted");
    assert.ok(implicit?.properties &&
        implicit.properties.CFrame &&
        implicit.properties.CFrame.__type === "CFrame");
    fs.rmSync(tmp, { recursive: true, force: true });
});
//# sourceMappingURL=rojo.test.js.map