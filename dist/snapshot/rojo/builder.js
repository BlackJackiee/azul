...
export class RojoSnapshotBuilder {
    ...
    moduleContainers = new Set();
    destPrefix;
    ignoreMatchers = [];
    constructor(options = {}) {
        this.cwd = path.resolve(options.cwd ?? process.cwd());
        this.projectFile = path.resolve(this.cwd, options.projectFile ?? "default.project.json");
    ...
    async build() {
        ...
        if ((!tree.$className || tree.$className !== "DataModel") &&
             ...
             */
            log.error(`Cannot build Rojo compatibility snapshot: project file does not have a DataModel root.`);
            log.error(`To fix this, either:`);
            log.error(`- Run "azul push" to specify a destination path that is not the root (e.g. "azul push -s . -d Workspace.${project.name || "RojoProject"} --rojo")`);
            log.error(`- Make sure the project file has a DataModel root (e.g. "tree": { "$className": "DataModel", ... })`);
        ...
        if (rootPath) {
            ...
            if (rootKind === "file") {
                if (hasChildren) {
                    throw new Error(`Root $path target ${absRoot} is a file and cannot define child nodes.`);
                }
                if (!isScriptFileName(path.basename(absRoot))) {
                    throw new Error(`$path target ${absRoot} must be a .lua or .luau script file.`);
                }
                const { className, scriptName } = classifyScriptFileName(path.basename(absRoot));
                ...
                const destPath = this.destPrefix.length === 0
                    ? [scriptName]
                ...
                await this.walkDirectory(absRoot, [...this.destPrefix], results, new Set(rootChildren));
            }
        }
        ...
        log.success(`Rojo compatibility build produced ${results.length} instances`);
        return results;
    }
    async loadProjectFrom(file) {
        let raw;
        try {
    ...
    async walkTree(node, parentPath, projectDir, results) {
    ...
    async parseModelFile(filePath, destPath) {
        let raw;
        try {
    ...
    async parseModelNode(node, name, currentPath, baseDir, results) {
    ...
    async emitNode(name, node, pathSegments, projectDir, results) {
        ...
        const definedChildren = new Set(Object.keys(node).filter((key) => !key.startsWith("$")));
        const pathKind = absPath ? await this.pathKind(absPath) : null;
        let initScript = null;
        let initModelFile = null;
            ...
            if (isInstanceJsonName(fileName)) {
                ...
                for (const [childName, childValue] of Object.entries(node)) {
                    ...
                    if (typeof childValue !== "object" || childValue === null)
                        continue;
                    await this.emitNode(childName, childValue, [...pathSegments, childName], projectDir, results);
                }
                return;
            }
            else if (this.isJsonModuleFile(fileName)) {
        ...
        if (initModelFile) {
            ...
            if (modelInstances.length > 0) {
                ...
                if (initScript) {
                    const scriptClass = initScript.className ??
    ...
    resolveClassName(node, pathSegments) {
    ...
    async walkDirectory(dirPath, destPath, results, definedChildren) {
        ...
        if (initModelEntry) {
            ...
            if (!this.moduleContainers.has(key)) {
                ...
                this.ensureFolder(destPath.slice(0, -1), results);
                const modelInstances = await this.parseModelFile(path.join(dirPath, "init.model.json"), destPath);
                if (modelInstances.length > 0) {
                    const rootInstance = modelInstances[0];
                    if (initEntry) {
                        handledEntries.add(initEntry.name);
                        const scriptClass = classifyScriptFileName(initEntry.name).className;
                        const source = await fs.readFile(path.join(dirPath, initEntry.name), "utf-8");
                        rootInstance.className = scriptClass;
        ...
        for (const entry of entries) {
            if (handledEntries.has(entry.name))
                continue;
            ...
            if (entry.isFile() &&
                ...
                if (modelInstances.length > 0) {
                    ...
                    if (companionScript) {
                        ...
                        const source = await fs.readFile(path.join(dirPath, companionScript.name), "utf-8");
                        rootInstance.className = scriptClass;
                        rootInstance.source = source;
                    }
                    results.push(...modelInstances);
            ...
            if (entry.isDirectory()) {
                await this.walkDirectory(fullPath, [...destPath, entry.name], results, new Set());
                continue;
            }
     ...
     */
    ensureFolder(pathSegments, results) {
        if (pathSegments.length === 0)
            return;
        const key = pathSegments.join("/");
        if (this.moduleContainers.has(key))
            return;
        if (this.emittedFolders.has(key))
            return;
        this.ensureFolder(pathSegments.slice(0, -1), results);
    ...
    isJsonModuleFile(fileName) {
        if (!fileName.endsWith(".json"))
            return false;
        if (fileName === "default.project.json")
            return false;
        if (fileName.endsWith(".model.json"))
            return false;
        return true;
    }
    async readJsonModuleSource(filePath) {
        let parsed;
        try {
    ...
    jsonToLuau(value, indent) {
        ...
        if (Array.isArray(value)) {
            ...
            const parts = value.map((entry) => `${childIndent}${this.jsonToLuau(entry, indent + 1)},`);
            return `{
${parts.join("\n")}
${indentStr}}`;
        }
        ...
        const parts = entries.map(([key, entryValue]) => {
            const formattedKey = this.isLuaIdentifier(key)
                ? key
                : `[${JSON.stringify(key)}]`;
            return `${childIndent}${formattedKey} = ${this.jsonToLuau(entryValue, indent + 1)},`;
        });
        return `{
${parts.join("\n")}
${indentStr}}`;
    }
    isLuaIdentifier(value) {
