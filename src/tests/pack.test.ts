...
test("PackCommand selects multiple Studio source trees and their ancestors", () => {
  ...
  try {
    ...
    const selected = (pack as any).selectSnapshotSources(snapshot);
    const selectedPaths = selected.map((instance: any) =>
      instance.path.join("/"),
    );
...
test("PushCommand reads a Studio source path directly from a sourcemap", () => {
  ...
  try {
    const instances = (push as any).buildPushInstancesFromSourcemap(
      "ReplicatedStorage.Assets",
      ["ReplicatedStorage", "Assets"],
      sourcemapPath,
    );
    ...
    assert.deepStrictEqual(
      instances.map((instance: any) => instance.path.join("/")),
      ["ReplicatedStorage/Assets/VFX", "ReplicatedStorage/Assets/VFX/Sparkles"],
    );
  } finally {
    (push as any).ipc.close();
...
test("PushCommand missing-only filtering preserves existing instances", () => {
  ...
  try {
    ...
    const filtered = (push as any).filterExistingInstances(
      mappings,
      existingInstances,
    );
    ...
    assert.deepStrictEqual(
      filtered[0].instances.map((instance: any) => instance.path.join("/")),
      ["ReplicatedStorage/Assets/VFX/Burst"],
    );
  } finally {
    (push as any).ipc.close();
  }
